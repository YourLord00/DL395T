...
class BSQ(torch.nn.Module):
    def __init__(self, codebook_bits: int, embedding_dim: int):
        ...
        self._encoder = torch.nn.Linear(embedding_dim, codebook_bits)
        self._decoder = torch.nn.Linear(codebook_bits, embedding_dim)
...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def __init__(self, patch_size: int = 5, latent_dim: int = 128, codebook_bits: int = 10):
        ...
        self.autoencoder = BSQ(codebook_bits=codebook_bits, embedding_dim=latent_dim)
    ...
    def encode_index(self, x: torch.Tensor) -> torch.Tensor:
        # raise NotImplementedError()
        return self.autoencoder._code_to_index(self.encode(x))

    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        # raise NotImplementedError()
        return self.decode(self.autoencoder._index_to_code(x))
